...
export class MainComponent implements OnInit, OnDestroy {
  ...
  private getPoLine(poLine: string) {
    ...
    this.restService.call(request).subscribe({
      next: (result) => {
        ...
        if (this.processType == "" && this.intUsers != undefined) {
          if (this.apiResult.interested_user[0].notify_cancel) {
            ...
            this.makeNextLoan(
              this.intUsers,
              this.apiResult.interested_user[0].primary_id
            );
          }
          ...
          this.alert.error(
            this.translate.instant("Translate.NoInterestedUser")
          );
          this.loading = false;
        } else {
      ...
      error: (e: RestErrorResponse) => {
        this.alert.error(
          e.message + this.translate.instant("Translate.FailGetPOLine")
        );
        console.error(e);
        this.loading = false;
      },
  ...
  private getLoan(inUser: any) {
    let url = `/bibs/${this.mms}/holdings/${this.holdingID}/items/${this.pid}/loans`;
    let activeUser = "";
    let request: Request = {
      url: url,
      method: HttpMethod.GET,
  ...
  private makeReturn(acUser: string, inUser: any) {
    let { mms, holdingID, pid, settings } = this;
    let { library, circDesc } = settings;
    let url = `/bibs/${mms}/holdings/${holdingID}/items/${pid}/?op=scan&library=${library}&circ_desc=${circDesc}&done=true`;
    let request: Request = {
      url: url,
  ...
  private makeNextLoan(value: any, activeUser) {
    ...
    for (let i = 0; i < inusers.length; i++) {
      if (inusers[i].primary_id == actUser && i != inusers.length - 1) {
        if (inusers[i + 1].notify_cancel) {
          this.user.id = inusers[i + 1].primary_id;
        } else {
          actUser = inusers[i + 1].primary_id;
        }
      }
    }
    if (this.user.id != "" && this.user.id != activeUser)
      this.getFullUser(this.user);
    else {
      this.barcode = "";
      const translatedEndRouting = this.translate.instant(
        "Translate.EndRouting"
      );
      const { itemLibrary, itemLocation } = this;
      const msg = `${translatedEndRouting}${itemLibrary} - ${itemLocation}</br>`;
      this.alert.info(msg);
      this.loading = false;
    }
  }
  ...
  private getFullUser(user: User) {
    ...
    this.restService.call(request).subscribe({
      next: (result) => {
        this.apiResult = result;
        user.first_name =
          this.apiResult.first_name != undefined
            ? this.apiResult.first_name
            : "";
        user.last_name =
          this.apiResult.last_name != undefined ? this.apiResult.last_name : "";
        user.default_address.city = this.apiResult.contact_info.address[0].city;

        for (const address of this.apiResult.contact_info.address) {
          if (address.preferred == true) {
            user.default_address.address_line1 = address.line1;
            user.default_address.address_line2 = address.line2;
            user.default_address.address_line3 = address.line3;
            user.default_address.address_line4 = address.line4;
            user.default_address.address_line5 = address.line5;
            user.default_address.city = address.city;
            user.default_address.postal_code = address.postal_code;
          }
        }
  ...
  private performLoan(user: User) {
    const url = `/users/${user.id}/loans/?item_barcode=${encodeURIComponent(
      this.barcode
    )}`;
    const requestBody = {
      circ_desk: {
    ...
    this.restService.call(request).subscribe({
      next: (result) => {
        ...
        this.alert.success(
          `Loan performed (${user.last_name} ${user.first_name})!`
        );
      ...
      error: (e: RestErrorResponse) => {
        this.alert.error(
          `Loan on user ${user.last_name} ${user.first_name} (${user.id}) failed!  ${e.message}`
        );
        this.showSkip = true;
        this.loading = false;
        console.error(e);
  ...
  getItem(value: any) {
    ...
    if (!barcode) {
      ...
      return this.alert.error(
        this.translate.instant("Translate.BarcodeInvalid")
      );
    }
  ...
  printSlipLetter(user: User, due_date: Date) {
    ...
    let formatted_content = `<b>${user.first_name} ${user.last_name}</b><br/><br/>`;
    if (user.default_address.address_line1 != null)
      formatted_content += user.default_address.address_line1 + "<br/>";
    if (user.default_address.address_line2 != null)
      formatted_content += user.default_address.address_line2 + "<br/>";
    if (user.default_address.address_line3 != null)
      formatted_content += user.default_address.address_line3 + "<br/>";
    if (user.default_address.address_line4 != null)
      formatted_content += user.default_address.address_line4 + "<br/>";
    if (user.default_address.address_line5 != null)
      formatted_content += user.default_address.address_line5 + "<br/>";
    if (user.default_address.postal_code != null)
      formatted_content += user.default_address.postal_code + " ";
    if (user.default_address.city != null)
      formatted_content += user.default_address.city + "<br/><br/>";
    formatted_content +=
      this.translate.instant("Translate.InformUser") + "<br/>";
    formatted_content +=
      "<br/><b>" + this.bibTitle + " (ISSN " + this.issn + ")";
    formatted_content += "<br/>" + this.itemDesc + "</b>";
    formatted_content +=
      "<br/>" +
      this.translate.instant("Translate.Library") +
      this.itemLibrary +
      " / " +
      this.itemLocation;
    ...
    formatted_content +=
      "<br/></br><b>" +
      this.translate.instant("Translate.DueDate") +
      due_date.toLocaleDateString("en-GB") +
      "</b>";
    let act_date = new Date();
    formatted_content +=
      '<br/><br/><p align="right"><i>' +
      act_date.toLocaleDateString("en-GB") +
      "</i></p>";
